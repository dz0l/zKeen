...
#[derive(Parser)]
#[command(name = "zkeen", before_help = "", about = "Веб-панель управления сервисом XKeen", disable_version_flag = true, disable_help_subcommand = true, styles = STYLES)]
struct Cli {
    #[arg(
        short = 'p',
        long = "port",
...
const ZKEEN_UI_INIT_CONTENT: &str = r#"#!/bin/sh
...
const ZKEEN_UI_LOG_C: &[u8] = b"/opt/var/log/zkeen-ui.log\0";

fn create_init() -> std::io::Result<()> {
    if let Some(dir) = Path::new(S99ZKEEN_UI).parent() {
        std::fs::create_dir_all(dir)?;
    }
    std::fs::write(S99ZKEEN_UI, ZKEEN_UI_INIT_CONTENT)?;
    std::fs::set_permissions(S99ZKEEN_UI, std::fs::Permissions::from_mode(0o755))
}

fn exec_init(cmd: &str) -> ! {
    use std::os::unix::process::CommandExt;
    let err = std::process::Command::new(S99ZKEEN_UI).arg(cmd).exec();
    eprintln!("{} {}", " Ошибка выполнения команды:".red().bold(), err);
    exit(1);
}

fn open_process_log() -> io::Result<std::fs::File> {
    if let Some(dir) = Path::new(ZKEEN_UI_LOG).parent() {
        std::fs::create_dir_all(dir)?;
    }
    OpenOptions::new().create(true).append(true).open(ZKEEN_UI_LOG)
}

fn setup_process_logging() {
    if let Err(e) = open_process_log() {
        eprintln!("{} {}: {}", " Не удалось открыть лог".red().bold(), ZKEEN_UI_LOG, e);
    }
    ...
    if !stdio_is_interactive() {
        if let Err(e) = redirect_stderr_to_process_log() {
            eprintln!("{} {}: {}", " Не удалось перенаправить stderr в".red().bold(), ZKEEN_UI_LOG, e);
        }
    }
}
...
async fn main() {
    if std::env::args().any(|arg| arg == "-v" || arg == "-V" || arg == "--version") {
        ...
        println!("  {}", format!("zKeen UI {}", VERSION).cyan().bold());
    ...
    if let Some(command) = cli.command {
        match command {
            Command::Setup => {
                eprintln!("{}", " Setup скрипт будет доступен в следующих версиях".yellow().bold());
                exit(0);
            }
            ...
            Command::CreateInit => {
                if let Err(e) = create_init() {
                    eprintln!("{} {}: {}", " Не удалось создать".red().bold(), S99ZKEEN_UI, e);
                    exit(1);
                }
                println!("\n{} создан {}\n", " Успех:".green().bold(), S99ZKEEN_UI);
                exit(0);
            }
            Command::ResetPassword => {
                ...
                if controller::get_pid("zkeen-ui")
                    .into_iter()
    ...
    println!("zKeen UI {} ({})", VERSION, get_arch());
    ...
    let state = AppState {
        ...
        debug: cli.debug,
        rci_token,
        listen_port: cli.port.parse().unwrap_or(7220),
    };
    ...
    let secure_api = Router::new()
        ...
        .route("/api/panel", get(panel::get_panel))
        .route("/api/panel/port", post(panel::post_panel_port))
