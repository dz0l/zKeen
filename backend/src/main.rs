...
#[derive(Parser)]
#[command(name = "zkeen-ui", before_help = "", about = "Веб-панель управления сервисом XKeen", disable_version_flag = true, disable_help_subcommand = true, styles = STYLES)]
struct Cli {
    #[arg(
        short = 'p',
        long = "port",
            ...
            Command::Start | Command::Stop | Command::Restart | Command::Status => {
                if !Path::new(S99ZKEEN_UI).exists() {
                    eprintln!(
                        ...
                        " Ошибка:".red().bold(),
                        "zkeen-ui create-init".green().bold()
                    );
                    exit(1);
                }
