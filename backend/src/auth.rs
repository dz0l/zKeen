...
pub async fn post_set_password(
    ...
    if password.len() < 4 {
        return (
            ...
            Json(ApiResponse::<()> {
                success: false,
                error: Some("password_too_short".into()),
                data: None,
            }),
        )
            .into_response();
    ...
    update_auth(&state, |auth| {
        auth.enabled = true;
        auth.password_hash = Some(hash);
        // Keep existing sessions so other open tabs stay logged in; add this one.
        auth.session_ids.push(session_val);
    })
    .await;

    (
        set_cookie_header(&headers, session_id, if req.remember { ttl } else { 0 }),
        Json(ApiResponse::<()> {
            success: true,
            error: None,
            data: None,
