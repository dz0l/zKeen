...
pub async fn get_control(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let xkeen_running = {
        let fallback = mihomo_running || xray_running;
        if tokio::fs::metadata("/opt/sbin/xkeen").await.is_err() {
            fallback
        } else {
            let mut cmd = tokio::process::Command::new("/opt/sbin/xkeen");
            cmd.arg("-status");
    ...
    let core_running = match current_core.name.as_str() {
        "mihomo" => mihomo_running,
        "xray" => xray_running,
        _ => mihomo_running || xray_running,
    };

    Json(serde_json::json!({
        "success": true,
        "cores": available_cores,
        "currentCore": current_core.name,
        "running": core_running,
        "mihomoRunning": mihomo_running,
        "xrayRunning": xray_running,
        "xkeenRunning": xkeen_running,
    }))
}
