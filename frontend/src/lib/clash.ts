export interface ClashProxyItem {
  ...
  /** Mihomo may expose snake_case in JSON. */
  "packet-encoding"?: string;
}

const PROXY_TYPE_SHORT: Record<string, string> = {
  hysteria2: "hy2",
  hysteria: "hy",
  shadowsocks: "ss",
  shadowsocksr: "ssr",
...
export function formatProxyTypeLabel(item: ClashProxyItem): string {
  ...
  const packetEnc = (item.packetEncoding || item["packet-encoding"] || "").toLowerCase();
  if (packetEnc) {
    return `${typeShort}/${packetEnc}`;
  }

  const network = (item.network || "").toLowerCase();
  if (network && network !== "tcp" && network !== rawType) {
    return `${typeShort}/${network}`;
  }

  if (item.udp) {
    return `${typeShort}/udp`;
  }
...
export function collectProviderServers(
  ...
  return collectProviderProxies(data, providerName).map((p) => p.name);
}

/** Provider nodes with protocol label for compact cards. */
export function collectProviderProxies(
  data: ClashProvidersResponse | null | undefined,
  providerName: string,
): ProviderProxy[] {
  const provider = data?.providers?.[providerName];
  const items: ProviderProxy[] = [];
  for (const item of provider?.proxies ?? []) {
    ...
    items.push({ name, typeLabel: formatProxyTypeLabel(item) });
  }
